...
test('Should return an amortization table from a single loan', () => {
    ...
    expect(loans.getAmortizationTable(monthlyPayment)).toMatchObject(amortizationTableFixtures.single);
});

test('Should return an amortization table from multiple loans', () => {
    const loan1 = new Loan('loan 1', 1000, 0.04, 10, 'loan1');
    const loan2 = new Loan('loan 2', 500, 0.05, 5, 'loan2');
    const loan3 = new Loan('loan 3', 2000, 0.06, 20, 'loan3');
    const loans = new LoanList(loan3, loan2, loan1);

    const monthlyPayment = 935;
    expect(loans.getAmortizationTable(monthlyPayment)).toMatchObject(amortizationTableFixtures.multiple);
});